...
#' Meta Data computation
#' @description Make data frame with one column name, 'data_id'.
#' data_id is a vector of length equals to total number
#' of cells in all matrices.
#'
#' @param data_list List of expression matrices
#' @param data_id List of names/ids of expression matrices
#'
#' @return data_id_df return dataframe with data_id column
#' @import Linnorm
                      ...
                      )
{
  ...
  data_id_df = as.data.frame(do.call("rbind", Batch_id))
  colnames(data_id_df) = "data_id"
  return(data_id_df)
}
...
#' Integrate single cell expression matrices present in the list
...
Data_integration = function(data_list)
{
  ...
  #If more than one expression data matrices in list
